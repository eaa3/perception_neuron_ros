#ifndef _ROS_cob_msgs_BatteryServer_h
#define _ROS_cob_msgs_BatteryServer_h
...
#include "cob_msgs/BatteryState.h"
...
namespace cob_msgs
{
  ...
  class BatteryServer : public ros::Msg
  {
    public:
      ...
      cob_msgs::BatteryState st_battery;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(battery_lengthT > battery_length)
        this->battery = (cob_msgs::BatteryState*)realloc(this->battery, battery_lengthT * sizeof(cob_msgs::BatteryState));
      offset += 3;
      battery_length = battery_lengthT;
      for( uint8_t i = 0; i < battery_length; i++){
      offset += this->st_battery.deserialize(inbuffer + offset);
        memcpy( &(this->battery[i]), &(this->st_battery), sizeof(cob_msgs::BatteryState));
      }
     return offset;
    }

    const char * getType(){ return "cob_msgs/BatteryServer"; };
